...
function callFunction(obj, comma)
{
  if(comma == undefined) comma = true;
	...
	switch(obj.expression.callee.type)
	{
      ...
      if(obj.expression.callee.name.indexOf("__NJS_") == 0 || obj.expression.callee.name.indexOf("__NATIVE_") == 0 || (COMPILER.LOCAL_PARAM.indexOf(obj.expression.callee.name) < 0 && COMPILER.CBFN.indexOf(obj.expression.callee.name) < 0))
      {
