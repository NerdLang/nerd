...
char* __NJS_STM32_strdup (const char* s)
{
...
inline char* __NJS_Concat_Int_To_Str(char* _left, int _right)
{
	...
	output << _left << _right;
	return __NJS_STM32_strdup(output.str().c_str());
}
...
inline var __NJS_Object_Set(var _index, var _value, var _array)
{
  if(_array.type == ARRAY)
  {
    ...
    if(_obj->size() <= _index.value.i) _obj->resize(_index.value.i * 1.5, var(UNDEFINED, 0));
...
__NJS_Class_Array::__NJS_Class_Array(vector<var>* _value)
{
  ...
  __NJS_Object_Set((char*)"length", var(_value->size()), this->__NJS_OBJECT);
