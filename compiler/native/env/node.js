...
var NODE =
{
  ...
  cli: function(compiler, preset, out, _in, option)
  {
	...
	if(_stack) _stack = "-Wl,--stack," + _stack;
	  ...
	  if(preset == "none")
	  {
		  return `${compiler} ${_stack} -std=c++17 ${_uvParam} ${_in} -O1 -fpermissive -w -s ${COMPILER.LIBS} ${_uvLib} -o a.exe && mv a.exe ${out}`;
	  }
	  else if(preset == "size")
	  {
		  return `${compiler} ${_stack} -std=c++17 ${_uvParam} ${_in} -Os -fno-rtti -fno-stack-protector -fomit-frame-pointer -fpermissive -w -s ${COMPILER.LIBS}  ${_uvLib} -o a.exe && mv a.exe ${out}`;
	  }
	  else
	  {
		var _opt = "-O";
            ...
            return `${compiler} ${_stack} -std=c++17 ${_uvParam} ${_in} ${_opt} -fpermissive -w -s ${COMPILER.LIBS}  ${_uvLib} -o a.exe && mv a.exe ${out}`;
	  }
  }
}
