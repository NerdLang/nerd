				...
				else if(_code[i] == "}")
				{
					...
					if(_count == 0)
					{
						...
						if(_match[1].indexOf("__MODULE") != 0)
						{
							...
							var _addNew = "__NJS_DECL_FUNCTION<NJS::VAR (vector<var>)>* " + _genNew +" = new __NJS_DECL_FUNCTION<NJS::VAR (vector<var>)>([" + _catch + "](vector<var> _NJS_VARARGS) -> NJS::VAR" + _fn + _returnThis + ");";
