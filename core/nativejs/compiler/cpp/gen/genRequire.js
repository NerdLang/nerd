      ...
      if(index > -1)
      {
        ...
        while(search)
        {
          if(src[target] == "'" || src[target] == "\"")
          {
            ...
            var modSource = "";
            if(addSource.indexOf(MAIN_PATH) > -1)
            {
              modSource = addSource;
            }
            else
            {
              modSource = from + addSource;
            }

            var trySource = [modSource, /*modSource + ".js",*/ modSource + "/" + "index.js", from + "nectar_modules/" + addSource + "/index.js", MAIN_PATH + "/nectar_modules/" + addSource + "/index.js", from + "node_modules/" + addSource + "/index.js", MAIN_PATH + "/node_modules/" + modSource + "/index.js" ];
            var newSrc = "";
            ...
            if(newSrc.length == 0) console.log("ERREUR REQUIRE : " + addSource);
            ...
            if(NJS_ENV.name != "arduino")
            {
              ...
              VARIABLE["MODULE"] = {};
              var reqFN = "__NJS_REQUIRE_" + createRandomVar();

              var resultSource = src.substring(0, index) + reqFN + "()";

              newSrc = "function " + reqFN + "(){\nvar MODULE = __create_Undefined();\n" + newSrc;
