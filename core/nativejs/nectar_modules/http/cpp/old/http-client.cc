...
namespace http {
  ...
  int Client::complete(http_parser* parser, Listener cb)
  {
    ...
    var res = var(OBJECT, new vector<shared_ptr<pair<var, var>>>());
    __NJS_Object_Set((char*)"body", var((char*)context->body.str().c_str()), res);
    //__NJS_Object_Set("parser", var(STRING, context->body.c_str()), res)
    //res.parser = *parser;
